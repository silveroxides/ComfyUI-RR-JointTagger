...
class Florence2Processor(ProcessorMixin):
    ...
    def __init__(
        ...
        new_tokens = ['<od>', '</od>', '<ocr>', '</ocr>'] + \
                    [f'<loc_{x}>' for x in range(1000)] + \
                    ['<cap>', '</cap>', '<ncap>', '</ncap>','<dcap>', '</dcap>', '<grounding>', '</grounding>', '<seg>', '</seg>', '<sep>', '<region_cap>', '</region_cap>', '<region_to_desciption>', '</region_to_desciption>', '<proposal>', '</proposal>', '<poly>', '</poly>', '<and>']
        
        # Robustly handle missing additional_special_tokens attribute
        existing_special_tokens = getattr(tokenizer, "additional_special_tokens", [])
        if not isinstance(existing_special_tokens, list):
            existing_special_tokens = []
            
        tokens_to_add = {
                'additional_special_tokens': list(set(existing_special_tokens + new_tokens))
            }
    ...
    def process_image(self, image: ImageInput, **kwargs) -> torch.Tensor:
        """
        Custom image processing that ensures the input is correctly formatted for the vision tower.
        """
        # Ensure we use the CLIPImageProcessor correctly
        processed = self.image_processor(image, return_tensors="pt", **kwargs)
